 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-improve-code/index
 * @returns eslint-config-improve-code/react
 * @returns eslint-config-improve-code/typescript/index
 * @returns eslint-config-improve-code/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-improve-code/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
