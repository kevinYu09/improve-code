...
> 注 1：事实上，你可以在任意目录执行 `improve-code-lint scan` `improve-code-lint` 会根据文件类型、JSON 等特征嗅探项目类型。但我们还是推荐在执行过 `improve-code-lint init` 的项目根目录执行 `improve-code-lint scan`，以得到最准确的扫描结果。
>
...
`improve-code-lint` 会读取执行目录下的 `improve-code-lint.config.js` 作为配置文件。`improve-code-lint init` 会在执行目录下新增如下的 `improve-code-lint.config.js` 文件：
